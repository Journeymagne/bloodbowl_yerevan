...
> **Выполнено 2026-08-22.** Все одиннадцать задач закрыты, семь критериев готовности
> достигнуты. Итоги и вывод внешних проверок — в
> [`docs/security-incident-2026-08-22.md`](../../security-incident-2026-08-22.md).
> Чекбоксы ниже оставлены неотмеченными намеренно: план пригоден для повторного
> прогона, если инцидент повторится.
>
> Что по ходу расходилось с планом: вход на сервер под `ubuntu` через `sudo`, а не
> под `root` (перенаправления в `/root` пришлось заворачивать в `sudo sh -c`);
> `node --test test/` в Node 22 трактует аргумент как файл, скрипт стал бесаргументным
> `node --test`; проверка дампа через `tail -1` попадала на пустую строку, нужен
> `tail -5`. Все три поправки внесены в текст ниже.
